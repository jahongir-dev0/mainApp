...
function deletebook(button1) {
    const cardElement = button1.parentElement.parentElement;
  
    cardElement.remove();
  

    const cardIndex = dataBook.findIndex(cardList => cardList.some(card => card.title === cardElement.querySelector('.card-title').textContent));
    if (cardIndex !== -1) {
      dataBook.splice(cardIndex, 1);
      localStorage.setItem('book', JSON.stringify(dataBook));
    } else {
      console.warn("Card not found in dataBook. Data consistency might be affected.");
    }
  }
            ...
            if (card.title && card.author && card.description && card.imageUrl) {
                let cardHTML = `
                    <div class="card-banner">
                        <figure class="img-holder" style="--width:60px; --height:300;">
                    ...
                    <div class="card-content">
                       ...
                       <button class="button1" onclick="deletebook(this)">Delete</button>
                    </div>
                `;
